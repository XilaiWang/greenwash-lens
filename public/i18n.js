(function () {
  const translations = {
    zh: {
      ...
      'settings.providerGroup': '主要 Provider',
      'settings.providerNone': '无（本地规则）',
      'input.tabText': '粘贴文本',
      'input.tabPdf': '上传 PDF',
      'settings.secondaryProviderGroup': '次要 Provider（分工分析）',
      'settings.secondaryProviderHint': '配置后，文本将被拆分为两部分，由主要和次要 Provider 并行分析。',
    ...
    en: {
      ...
      'settings.providerGroup': 'Primary provider',
      'settings.providerNone': 'None (local rules)',
      'input.tabText': 'Paste text',
      'input.tabPdf': 'Upload PDF',
      'settings.secondaryProviderGroup': 'Secondary provider (split analysis)',
      'settings.secondaryProviderHint': 'When configured, text is split in two halves analyzed in parallel by primary and secondary providers.',
      'settings.apiKeyLabel': 'API Key',
